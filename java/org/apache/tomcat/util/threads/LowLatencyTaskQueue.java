...
/**
 * As task queue specifically designed to run with the low-latency thread pool
 * executor. The task queue is optimised to properly utilize threads within
 * a thread pool executor.
 * <p>
 * If you use a normal queue, the executor will spawn threads when
 * there are idle threads and you won't be able to force items onto the queue
 * itself.
 */
public class LowLatencyTaskQueue extends LinkedBlockingQueue<Runnable> {
    ...
    public LowLatencyTaskQueue() {
        super();
    }

    public LowLatencyTaskQueue(int capacity) {
        super(capacity);
    }

    public LowLatencyTaskQueue(Collection<? extends Runnable> c) {
        super(c);
    }

    public void setParent(LowLatencyThreadPoolExecutor tp) {
        parent = tp;
    }
